...
def creat_searchspace(consider_runs):
    ...
    for i in consider_runs:
        try:
            run_dic_list.append(openmlrun_dic_step1.run_to_dic(i))

            if len(openmlrun_dic_step1.run_to_dic(i))>=1:
                for key in openmlrun_dic_step1.run_to_dic(i):
                    if key not in search_space:
                        search_space[key] = [openmlrun_dic_step1.run_to_dic(i)[key]]
                    else:
                        if openmlrun_dic_step1.run_to_dic(i)[key] in search_space[key]:
                            pass
                        else:
                            search_space[key] = search_space[key]+[openmlrun_dic_step1.run_to_dic(i)[key]]
        except:
                print('except')
    print(search_space)
    return search_space
